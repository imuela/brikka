import { TestBed } from '@angular/core/testing';
import { HttpClient, HttpContext, provideHttpClient, withInterceptors } from '@angular/common/http';
import { HttpTestingController, provideHttpClientTesting } from '@angular/common/http/testing';

import { environment } from '../../../environments/environment';
import { PortalAuthService } from '../../portal-auth/portal-auth.service';
import { portalAuthInterceptor } from './portal-auth.interceptor';
import { SKIP_AUTH } from './http-context';

describe('portalAuthInterceptor', () => {
  let http: HttpClient;
  let httpMock: HttpTestingController;
  let portalAuthService: PortalAuthService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [
        provideHttpClient(withInterceptors([portalAuthInterceptor])),
        provideHttpClientTesting(),
      ],
    });
    http = TestBed.inject(HttpClient);
    httpMock = TestBed.inject(HttpTestingController);
    portalAuthService = TestBed.inject(PortalAuthService);
  });

  afterEach(() => httpMock.verify());

  it('attaches the Portal Bearer token to /api/v1/portal/** requests', () => {
    vi.spyOn(portalAuthService, 'accessToken').mockReturnValue('portal-token-123');

    http.get(`${environment.apiBaseUrl}/api/v1/portal/me`).subscribe();

    const req = httpMock.expectOne(`${environment.apiBaseUrl}/api/v1/portal/me`);
    expect(req.request.headers.get('Authorization')).toBe('Bearer portal-token-123');
    req.flush({});
  });

  it('does not attach a header when there is no Portal token', () => {
    vi.spyOn(portalAuthService, 'accessToken').mockReturnValue(null);

    http.get(`${environment.apiBaseUrl}/api/v1/portal/me`).subscribe();

    const req = httpMock.expectOne(`${environment.apiBaseUrl}/api/v1/portal/me`);
    expect(req.request.headers.has('Authorization')).toBe(false);
    req.flush({});
  });

  it('never attaches the Portal token to an internal request', () => {
    vi.spyOn(portalAuthService, 'accessToken').mockReturnValue('portal-token-123');

    http.get(`${environment.apiBaseUrl}/api/v1/me`).subscribe();

    const req = httpMock.expectOne(`${environment.apiBaseUrl}/api/v1/me`);
    expect(req.request.headers.has('Authorization')).toBe(false);
    req.flush({});
  });

  it('never attaches the token to a request marked SKIP_AUTH', () => {
    vi.spyOn(portalAuthService, 'accessToken').mockReturnValue('portal-token-123');

    http
      .get('http://localhost:18081/realms/brika-portal/protocol/openid-connect/token', {
        context: new HttpContext().set(SKIP_AUTH, true),
      })
      .subscribe();

    const req = httpMock.expectOne(
      'http://localhost:18081/realms/brika-portal/protocol/openid-connect/token',
    );
    expect(req.request.headers.has('Authorization')).toBe(false);
    req.flush({});
  });
});
