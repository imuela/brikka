...
export function statusTone(value: string | null | undefined): StatusTone {
  ...
  if (
    /(REJECTED|CANCELLED|CANCELED|ERROR|FAILED|OVERDUE|EXPIRED|DECLINED|SUSPENDED|BLOCKED|LOST)/.test(
      v,
    )
  ) {
    return 'error';
  ...
  if (
    /(PENDING|IN_REVIEW|IN_PROGRESS|DRAFT|REQUESTED|SUBMITTED|WAITING|SCHEDULED|TRIAL)/.test(v)
  ) {
    return 'warning';
  }
  if (
    /(APPROVED|ACTIVE|COMPLETED|ACCEPTED|CLOSED_WON|WON|PAID|VERIFIED|CONFIRMED|ENABLED)/.test(v)
  ) {
    return 'success';
  }
