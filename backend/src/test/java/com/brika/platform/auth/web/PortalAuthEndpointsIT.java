...
/**
 * Portal Cliente equivalent of {@link UserAuthEndpointsIT} — same login/refresh/logout/
 * change-password/lockout scenarios, exercised against {@code /api/v1/portal/auth/**}
 * (ADR-PORTAL-AUTH-001: never sharing a service/table with the internal realm). Sprint 22 cierre
 * punto 11: this file did not exist before — only {@link SelfIssuedAuthEndToEndIT} exercised a
 * single happy-path Portal login, so refresh-reuse-detection, lockout, and change-password were
 * never actually verified against the Portal endpoints.
 */
@Testcontainers
@SpringBootTest
@AutoConfigureMockMvc
@Import(StubJwtDecoderConfig.class)
class PortalAuthEndpointsIT {
  ...
  private record Fixture(UUID portalAccountId, String externalIdentityId, String email) {
    String bearer() {
      return "Bearer " + externalIdentityId;
    }
  }

  private Fixture seedPortalAccount(String emailPrefix, String password) {
    UUID companyId =
        companyRepository.insert("Co " + emailPrefix, "Co " + emailPrefix, "TC-" + emailPrefix);
    String email = emailPrefix + "@brika.test";
    UUID clientId = clientRepository.insert(companyId, "First", "Last", email, "000000000");
    String externalId = "ext-portal-" + UUID.randomUUID();
    UUID portalAccountId =
        portalAccountRepository.insert(companyId, clientId, externalId, "ACTIVE");
    portalAccountCredentialService.setPassword(portalAccountId, password);
    return new Fixture(portalAccountId, externalId, email);
  }
  ...
  void loginWithCorrectCredentialsIssuesAccessAndRefreshTokens() throws Exception {
    Fixture fixture = seedPortalAccount("portal-login-ok", "Correct-Horse-1");
  ...
  void loginWithWrongPasswordIsRejected() throws Exception {
    Fixture fixture = seedPortalAccount("portal-login-wrong", "Correct-Horse-2");
    String body =
        objectMapper.writeValueAsString(new LoginApiRequest(fixture.email(), "totally-wrong"));

    mockMvc
        .perform(
            post("/api/v1/portal/auth/login").contentType(MediaType.APPLICATION_JSON).content(body))
  ...
  void loginWithUnknownEmailIsRejectedWithTheSameStatusAsWrongPassword() throws Exception {
    String body =
        objectMapper.writeValueAsString(
            new LoginApiRequest("no-such-client-" + UUID.randomUUID() + "@brika.test", "whatever"));
  ...
  void refreshRotatesTheTokenAndInvalidatesThePreviousOne() throws Exception {
    Fixture fixture = seedPortalAccount("portal-refresh-ok", "Correct-Horse-4");
    ...
    // Reuse detection must revoke the entire family, not just the token that was replayed.
  ...
  void logoutInvalidatesTheRefreshToken() throws Exception {
    Fixture fixture = seedPortalAccount("portal-logout-ok", "Correct-Horse-5");
  ...
  void changePasswordRequiresCurrentPasswordAndRevokesExistingRefreshTokens() throws Exception {
    Fixture fixture = seedPortalAccount("portal-chpwd-ok", "Correct-Horse-6");
  ...
  void loginIsLockedOutAfterTooManyFailedAttempts() throws Exception {
    Fixture fixture = seedPortalAccount("portal-lockout", "Correct-Horse-7");
    String wrongBody =
