 ...
 * and Flyway must run every migration (V1-V13) successfully. This is the same physical schema
 ...
 * permissions. V8-V12 add no table; V13 adds two. The exact role_permissions content is verified in
 * RbacSeedIT.
 */
@Testcontainers
@SpringBootTest
class FlywayMigrationIT {
  ...
  void contextLoadsAndFlywayMigratesEveryTable() {
    ...
    // V13 adds bank_match_results + bank_match_rule_results = 50.
    assertThat(tableCount).isEqualTo(50);
    ...
    assertThat(permissionCount)
        .isEqualTo(113); // 110 full atomic catalog (14_DEFINITIVE_PERMISSION_CATALOG.md) + 1
    // CLIENT_PORTAL_ACCOUNT_CREATE (ADR-PORTAL-AUTH-001, V11) + 2 BANK_MATCHING_RUN/READ
    // (ADR-BANKENGINE-001, V13)
    ...
    // (SUPERADMIN/MANAGER/BROKER x BANK_MATCHING_RUN/READ, V13) = 229. Full breakdown and
    // PENDING/NOT_ASSIGNED absence verified in RbacSeedIT.
    assertThat(rolePermissionCount).isEqualTo(229);
